...
#include "stm32f10x.h"
#include <stdbool.h>
static unsigned char reday_to_receive=0,count=1,state=1;
  ...
  while(1)
  {
		...
		if(count<3){
			if(count==1){
				...
				leftWheel(5000,FORWARD);
				rightWheel(300,BACKWARD);
				DelayMs(350);
				while(count==2){
					readFloor();
					LED8(reading);
					leftWheel(4500,FORWARD);
					rightWheel(4500,BACKWARD);
					if(reading&(1<<7)){
						state=2;
						count++;
						countT=0;
			...
			if(reading==0xFF||(reading&(1<<4)&&reading&(1<<3)&&reading&(1<<2)&&reading&(1<<1)&&reading&(1<<0))||(reading&(1<<7)&&reading&(1<<6)&&reading&(1<<5)&&reading&(1<<4)&&reading&(1<<3))){
				if(countT>2400){
					count++;
					countT=0;
				}
				if(count==3){
					lnrWheel(3000,FORWARD);
				}else if(count==4){
					leftWheel(220,BACKWARD);
					rightWheel(4000,FORWARD);
					DelayMs(400);
					while(count==4){
						readFloor();
						LED8(reading);
						leftWheel(4200,BACKWARD);
						rightWheel(4200,FORWARD);
						if(reading&(1<<0)||reading&(1<<3)){
							state=3;
							count++;
						}
					}
				}else if(count>6){
					lnrWheel(1600,FORWARD);
					DelayMs(700);
					lnrWheel(0,FORWARD);
					while(1);
				}else{
					rightWheel(2000,FORWARD);
					leftWheel(2000,FORWARD);
					state=1;
				}
			}
		}
		if(state==1){
			...
			if(reading&(1<<3)||reading&(1<<2)||reading&(1<<1)){
				state=2;	
			}
			if(reading&(1<<6)||reading&(1<<5)||reading&(1<<4)){
				state=3;
			}
		}else if(state==2){	//turnLeft
			rightWheel(2600,FORWARD);
			leftWheel(1500,FORWARD);
			if(reading&(1<<1)){
				rightWheel(3400,FORWARD);
				leftWheel(100,FORWARD);
			}
			if(reading&(1<<0)){
				rightWheel(4300,FORWARD);
				leftWheel(300,BACKWARD);
			}
			if(reading==0){
				rightWheel(5500,FORWARD);
				leftWheel(5500,BACKWARD);
			}
			DelayMs(60);	//angle
			lnrWheel(2800,FORWARD);
			...
			if(reading&(1<<7)){
				rightWheel(4300,BACKWARD);
				leftWheel(300,FORWARD);
			}
			if(reading==0){
				rightWheel(5500,BACKWARD);
				leftWheel(5500,FORWARD);
			}
			DelayMs(60);	//angle
			lnrWheel(3000,FORWARD);
			if(reading&(1<<1)||reading&(1<<2)||reading&(1<<3)||reading&(1<<4)){
				state=1;
			}
		}
